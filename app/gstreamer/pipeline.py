...
class Pipe(object):
    def __init__(self, name, rtsp_url):
        ...
        #self.pipeline_command = ("rtspsrc location={} protocols=tcp name=rtsp_src tcp-timeout=10000000 ! decodebin ! nvvidconv output-buffers=1 ! videorate drop-only=true ! video/x-raw,framerate=1/1 ! nvjpegenc quality=95 ! appsink name=app_sink drop=true max-buffers=8").format(rtsp_url)
